...
Always close the block with a matching `:::` and pick the tone that fits the
message (`note`, `tip`, `warning`, or `danger`).
...
For components in `docs/<locale>/_partials/` or one-off snippets, use explicit imports at the top
of the file:
